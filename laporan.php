...
<div class="form">
    ...
    <form name="Laporan" action='#' method='post' onsubmit="return validasi()" border='0'>
        <table class='tabel2'>
            ...
            <tr>
                <td>Tanggapan</td>
                <td><textarea rows="4" cols="50" value='<?php echo $tanggapan; ?>' name='tanggapan'></textarea></td>
            </tr>
            <tr>
            </tr>
                <td>Status</td>
...
<?php
    ...
    //real_escape_string digunakan untuk menghindari serangan pada database seperti SQL injection
    $query = "DELETE from pengaduan where id_pengaduan = ".$mysqli->real_escape_string($_GET['id'])."";
    //Mengeksekusi Query
    if($mysqli->query($query)){
        
    }else{
        //Apabila Penghapusan Gagal
        echo "\n<a>Database error : Data gagal dihapus</a>";
    }
    }
...
<script type='text/javascript'>
    ...
    function delete_data (id_pengaduan){
        ...
        if(answer){ //Apabila user memilih ok
            ...
            window.location='laporan.php?action=delete&id=' + id_pengaduan;
        }
    }
</script>
